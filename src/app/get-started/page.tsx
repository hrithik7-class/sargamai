...
export default function GetStartedPage() {
  ...
  // Read error from URL when redirected from NextAuth error page
  const errParam = searchParams.get("error");
  const descParam = searchParams.get("error_description");
  const urlError =
    errParam || descParam
      ? (() => {
          const decoded = descParam ? decodeURIComponent(descParam.replace(/\+/g, " ")) : null;
          const raw = decoded ?? ERROR_MESSAGES[errParam ?? ""] ?? errParam ?? "Something went wrong.";
          return raw === "An unexpected error occurred" ? GENERIC_ERROR_FRIENDLY : raw;
        })()
      : null;

  // Show URL error; persist to state and clear URL (sync external URL to React state)
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional: run when URL has error params
  }, [errParam, descParam]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      if (result.error.startsWith("VERIFICATION_REQUIRED:")) {
  ...
  return (
    <div className="min-h-screen flex relative">
      ...
      {/* RIGHT SIDE - Form */}
      <div className="w-full lg:w-[55%] flex items-center justify-center p-4 sm:p-6 lg:p-12 bg-neutral-500 min-h-screen lg:min-h-0">
        <div className="w-full max-w-md">
          ...
          <div className="flex justify-center">
            <button
              ...
              className="flex items-center justify-center gap-2 py-2.5 px-6 rounded-lg border border-lavender-500 hover:bg-lavender-700 hover:border-teal-800 transition-colors text-sm font-medium text-jet-black"
            >
